...
export default function Pakan() {
  ...
  // Find the specific item for "STOK PAKAN"
  const pakanData = cardData.find(item => item.title === "STOK PAKAN");
        ...
        <div className="relative w-full max-w-3xl flex flex-col items-center">
          ...
          {pakanData ? (
            <div className="bg-white p-6 border border-gray-300 rounded-lg shadow-md w-full max-w-md mb-6">
              <h2 className="text-2xl font-bold mb-4 text-gray-800 text-center">Stok Pakan Data</h2>
              <div className="bg-gray-50 p-4 border border-gray-300 rounded-lg">
                <h3 className="text-lg font-semibold text-gray-700">{pakanData.title}</h3>
                <p className="text-gray-600">{pakanData.description}</p>
              </div>
            </div>
          ) : (
            <div className="bg-white p-6 border border-gray-300 rounded-lg shadow-md w-full max-w-md mb-6 text-center">
              <p className="text-gray-600">No data available for Stok Pakan.</p>
            </div>
          )}
        </div>
